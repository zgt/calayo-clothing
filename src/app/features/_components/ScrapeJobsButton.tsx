...
export default function ScrapeJobsButton({ disabled, onJobStarted }: ScrapeJobsButtonProps) {
    ...
    <div className="relative">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg bg-emerald-700 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed"
        >
      ...
      {/* Settings Panel */}
      {showSettings && (
        <div className="absolute top-full left-0 mt-2 w-80 rounded-lg border border-emerald-700/20 bg-emerald-950/95 p-4 shadow-2xl backdrop-blur-sm z-10">
          <h3 className="font-medium text-white mb-3">Scraping Settings</h3>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-md border border-emerald-700/20 bg-emerald-900/30 px-3 py-2 text-white placeholder-emerald-400/50 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
              />
            ...
            <div>
              <label className="flex items-center gap-2">
                ...
                />
                <span className="text-sm text-emerald-200">Skip duplicate jobs</span>
              </label>
              <p className="text-xs text-emerald-200/60 mt-1 ml-6">
                Avoid adding jobs that already exist in the sheet
              </p>
            </div>

            {/* Actions */}
            <div className="flex items-center gap-2 pt-2 border-t border-emerald-700/20">
              <button
                onClick={handleStartScraping}
                disabled={disabled || scrapeJobsMutation.isPending}
              ...
              >
                {scrapeJobsMutation.isPending ? "Starting..." : "Start Scraping"}
              </button>
              <button
